...
class Game:
    ...
    def play_game(self):
        ...
        while self.board.get_coins_left():
            ...
            best_score, best_move = self.minimax(depth=3, player_index=self.player_index, is_maximizing=1-self.player_index, alpha=-float('inf'), beta=float('inf'), board=self.board, players=self.players)
            if best_move:
    ...
    def minimax(self, depth, player_index, is_maximizing, alpha, beta, board, players):
        ...
        if is_maximizing:
            ...
            for move in players[player_index].get_valid_moves(board, players):
                new_board, new_player = self.simulate_move(move, player_index, board, players)
                evaluation, _ = self.minimax(depth - 1, 1 - player_index, False, alpha, beta, new_board, players.copy())
            ...
            for move in players[player_index].get_valid_moves(board, players):
                new_board, new_player = self.simulate_move(move, player_index, board, players)
                evaluation, _ = self.minimax(depth - 1, 1 - player_index, True, alpha, beta, new_board, players.copy())
    ...
    def simulate_move(self, move, player_index, board, players):
        ...
        new_player = Player(players[player_index].position, players[player_index].score)
        new_player.position = (new_player.position[0] + move[0], new_player.position[1] + move[1])
        # Apply the move on the cloned board and player
        if new_board.board[new_player.position] == 1:
            new_player.score += 1
            new_board.board[new_player.position] = 0
        # Return the new board and player
    ...
    def apply_move(self, move, player, board=None):
        ...
        if board.board[new_position] == 1:
            ...
            if player.consecutive_coins >=3 and BONUS: # Take in account the current coin streak and apply bonus accordingly
